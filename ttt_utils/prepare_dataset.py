...
import numpy as np # stliu

NORM = ((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
te_transforms = transforms.Compose([transforms.ToTensor(),
									transforms.Normalize(*NORM)])
tr_transforms = transforms.Compose([transforms.RandomCrop(32, padding=4), # transforms.RandomResizedCrop(32), # stliu: this may improve the performance
...
common_corruptions = ['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur', 'glass_blur',
					...
					'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression', 'upsample']
...
def prepare_test_data(args):
	if args.dataset == 'cifar10':
		...
		if not hasattr(args, 'corruption') or args.corruption == 'original':
			...
			# stliu: use our data
			teset_raw = np.load( '../data/myCIFAR-10-C/CIFAR-10-C-trainval/val/%s_%s_images.npy' %(args.corruption, str(args.level-1)) )
