...
def gaussian_noise_con(x, severity=5):
	c = random.uniform(0, 0.1)

	x = np.array(x) / 255.
	return ( np.clip(x + np.random.normal(size=x.shape, scale=c), 0, 1) * 255 ).astype(np.uint8)

# stliu: according to https://arxiv.org/abs/2001.06057, this is the best for corruption
def gaussian_noise_05(x, severity=5):
	c = 0.5
...
class make_augmentation(object):
	def __init__(self, cfunc, level=5):
		self.cfunc = cfunc
		self.level = level

	def __call__(self, img):
		if type(self.cfunc) == list:
			img = random.choice(self.cfunc)(img, self.level)
		else:
			img = self.cfunc(img, self.level)
		return img