...
def parse_option(): # design a function for parse
	...
	opt.model_name = 'supervised_w{}_{}_lr_{}_bsz_{}_aug_{}'.format(opt.width, opt.arch, opt.lr, opt.batch_size, opt.aug)
...
def main_worker(gpu, ngpus_per_node, args):
	...
	# stliu: use resnet_ttt
	if args.arch == 'resnet_ttt':
		model = ResNetCifar(width=args.width)
	else:
	...
	# Data loading code
	# traindir = os.path.join(args.data, 'train') # stliu: removed
	# valdir = os.path.join(args.data, 'val')
	# normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
	#                                  std=[0.229, 0.224, 0.225])
	normalize = transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],
									std=[0.247, 0.243, 0.261]) # stliu: for cifar

	# stliu: change to the same augment as moco
	...
	train_dataset = datasets.CIFAR10(root=args.data, train=True, download=True, \
		transform=transforms.Compose(aug(args.aug)))
	...
	val_data = datasets.CIFAR10(root=args.data, train=False, download=True, transform=test_transform)
	if args.corruption != 'original':
		corruption, level = args.corruption.split(',')
		teset_raw = np.load(args.data + '/CIFAR-10-C-trainval/val/%s.npy' %(corruption))[(int(level)-1)*10000: int(level)*10000]
		val_data.data = teset_raw
	val_loader = torch.utils.data.DataLoader(val_data, batch_size=args.batch_size, 
								shuffle=False, num_workers=args.workers, pin_memory=True)

	# stliu: test the resume model
	if args.evaluate:
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		logger.log_value('loss', loss, epoch) # stliu: add loss

		# evaluate on validation set
		acc1 = validate(val_loader, model, criterion, args)
		logger.log_value('acc1', acc1, epoch) # stliu: add acc1
		...
		if not args.multiprocessing_distributed or (args.multiprocessing_distributed
			...
			if epoch % args.save_freq == 0:
				...
				}, is_best, args.model_folder, filename='/checkpoint_{:04d}.pth.tar'.format(epoch))
...
def validate(val_loader, model, criterion, args):
	...
	with torch.no_grad():
		...
		print(args.corruption + ' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}'
